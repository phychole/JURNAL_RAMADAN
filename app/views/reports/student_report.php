<h4 class="mb-3 d-print-none">Report Siswa</h4>

<?php
  $info = $studentInfo ?? [];
  $nis = $info['nis'] ?? '-';
  $classLabel = $info['class_label'] ?? '-';

  $totalDays = is_array($details ?? null) ? count($details) : 0;
  $filled = 0;
  foreach (($details ?? []) as $d) {
    if (!empty($d['journal']) || !empty($d['sermon']) || !empty($d['good']) || !empty($d['extra'])) $filled++;
  }
?>

<!-- Header khusus print (Resmi) -->
<div class="print-header mb-2">
  <?php
    // Tanggal cetak format Indonesia sederhana
  ...
  <div class="kop-grid">
    <div>
      <img class="kop-logo" src="<?= u('public/assets/logo.png') ?>" alt="Logo Sekolah">
    </div>
    <div class="kop-text">
      <div class="kop-wrap">
  ...
  <table class="table table-borderless table-sm mb-1" style="width:auto;">
    ...
    <tr><td><b>Nama Siswa</b></td><td>: <?= htmlspecialchars((string)($info['name'] ?? $user['name'])) ?></td></tr>
    <tr><td><b>Kelas</b></td><td>: <?= htmlspecialchars((string)$classLabel) ?></td></tr>
    <tr><td><b>Periode</b></td><td>: Hari <?= (int)$fromDay ?> s/d <?= (int)$toDay ?> Ramadan <?= htmlspecialchars(RAMADAN_HIJRI_YEAR) ?> H</td></tr>
  </table>
...
<div class="card shadow-sm d-print-none mb-3">
  <div class="card-header brand d-flex flex-wrap gap-2 justify-content-between align-items-center">
    <div class="fw-semibold">Periode: Hari <?= (int)$fromDay ?> s/d <?= (int)$toDay ?> Ramadan <?= htmlspecialchars(RAMADAN_HIJRI_YEAR) ?> H</div>
    <div class="d-flex gap-2">
      <a class="btn btn-sm btn-outline-success" href="<?= u('/student/report/xls') ?>?from_day=<?= (int)$fromDay ?>&to_day=<?= (int)$toDay ?>">Export XLS</a>
      <button type="button" class="btn btn-sm btn-outline-secondary" onclick="window.print()">Cetak (A4)</button>
    </div>
  </div>
  <div class="card-body">
    <div class="row g-2">
      <div class="col-md-4"><div class="small text-muted">NISN/NIS</div><div><?= htmlspecialchars((string)$nis) ?></div></div>
      <div class="col-md-4"><div class="small text-muted">Nama</div><div><?= htmlspecialchars((string)($info['name'] ?? $user['name'])) ?></div></div>
      <div class="col-md-4"><div class="small text-muted">Kelas</div><div><?= htmlspecialchars((string)$classLabel) ?></div></div>
      <div class="col-12"><div class="small text-muted">Terisi</div><div><?= (int)$filled ?> / <?= (int)$totalDays ?></div></div>
    </div>
  </div>
</div>

<div class="card shadow-sm">
  <div class="card-header brand fw-semibold">Detail Kegiatan Harian</div>
  <div class="card-body">
            ...
            <tr>
              ...
              <td>
                <div class="small">
                  ...
                  Tadarus (hlm): <?= $j ? (int)($j['tadarus_pages'] ?? 0) : 0 ?><br>
...
<div class="d-none d-print-block">
  ...
  <table class="sign-table">
    ...
    <tr>
      <td style="text-align:center; white-space:pre-line;"><?= nl2br(htmlspecialchars($fmt($hom))) ?></td>
      <td style="text-align:center; white-space:pre-line;"><?= nl2br(htmlspecialchars($fmt($rel1))) ?></td>
      <td style="text-align:center; white-space:pre-line;"><?= nl2br(htmlspecialchars($fmt($kep))) ?></td>
    </tr>
  </table>
</div>
